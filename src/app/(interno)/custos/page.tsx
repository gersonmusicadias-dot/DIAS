...
export default async function PaginaCustos() {
  ...
  const custos = custosBrutos.map((c) => {
    ...
    return { id: c.id, descricao: c.descricao, tipo: c.tipo, categoria: c.categoria?.nome ?? null, categoriaId: c.categoriaId, competencia: c.competencia, vencimento: c.vencimento, previsto: f.previsto, pago: f.pago, saldo: f.saldo, situacao: f.pago <= 0 ? "PREVISTO" : f.saldo > 0 ? "PARCIAL" : "PAGO", temMovimento: c.pagamentos.length > 0 };
  });
